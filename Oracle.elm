...
moduleToDocs modul { alias, exposed } =
    let
        ...
        typeToPair oneType tag =
            let
                fullName =
                    modul.name ++ "." ++ tag

                info =
                    Info tag fullName (urlTo oneType) "" ""
            in
                [ ( tag, info )
                , ( fullName, info )
                ]
    in
        List.concatMap nameToPair (modul.aliases ++ modul.values)
            ++ List.concatMap (\oneType -> List.concatMap (typeToPair oneType.name) oneType.cases) modul.types
